...
def datetime_to_str_formt(cur_time: float):
    """ 将datetime的时间戳转换成固定格式的字符串  """
...
def datetime_2string(dt=None, fmt='%Y-%m-%d %H-%M-%S'):
    """将datetime类型转换为字符串"""
    if not dt:
...
def datetime_2string_simple(dt=None, fmt='%Y-%m-%d'):
    """将datetime类型转换成字符串"""
    if not dt:
        dt = datetime.datetime.now()
    return datetime.datetime.strftime(dt, fmt)
...
def string_2dtatetime3(dt_str, fmt='%Y-%m-%dT%H:%M:%S'):
    st = datetime.datetime.strptime(dt_str, fmt)
